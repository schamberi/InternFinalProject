            ...
            for index, row in excel_data.iterrows():
                ...
                if any(excel_data.columns.str.contains('code')):
                    ...
                    for value in values:  # Iterate through the values
                        ...
                        sector_standard_match = SectorStandards.objects.filter(code__iexact=lower_value).first()
                    ...
                    for value in values:  # Iterate through the values
                        lower_value = str(value).lower()
                        sector_standard_match = SectorStandards.objects.filter(sector_label__iexact=lower_value).first()
                        if sector_standard_match:
                            id_of_matched_sector_standard = sector_standard_match.id
                            sector_standard_ids[index] = id_of_matched_sector_standard  # Store the ID in the dictionary
                            break
...
from .models import ExcelFile, Company, SectorStandards, Country
import pandas as pd

class StandartDataNewTwo(APIView):
    template_name = 'uploadButon.html'
